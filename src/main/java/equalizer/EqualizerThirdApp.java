...
public class EqualizerThirdApp {
    ...
    public EqualizerThirdApp() {
        pool = Executors.newFixedThreadPool(COUNT_OF_THREADS);
        this.createFilters();
    }

    public void setInputSignal(short[] inputSignal, String filterLevel) {
        this.outputSignal = new short[inputSignal.length];
        if (filterLevel.equals("Bad")) {
            this.filters[0].settings(inputSignal, FilterInfo.COFFS_NUM_OF_BAND_0_BAD);
        } else if (filterLevel.equals("Medium")) {
            this.filters[0].settings(inputSignal, FilterInfo.COFFS_NUM_OF_BAND_0_MEDIUM);
        } else {
